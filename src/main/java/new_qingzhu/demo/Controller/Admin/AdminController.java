...
import new_qingzhu.demo.VO.OrderListVO;
...
public class AdminController {
    ...
    @Autowired
    public AdminController(AdminUserService adminUserService,OrderService orderService){
    ...
    public String datadog() {
        return "admin/DataShow";
    }

    @GetMapping({"/DataShow/data"})
    @ResponseBody
    public String data(){
        List<OrderListVO> orderList=orderService.getAllOrderList();

        if(!orderList.isEmpty()){
            return ResultGenerator.genSuccessResult().toString()+ JSON.toJSONString(orderList);
        }
    ...
    public String login(@RequestParam("userName") String userName,
        ...
        TQingzhuAdminUser adminUser = adminUserService.login(userName, password);
        if (adminUser != null) {
    ...
    public String profile(HttpServletRequest request) {
        ...
        TQingzhuAdminUser adminUser = adminUserService.getUserDetailById(loginUserId);
        if (adminUser == null) {
            return "admin/login";
        }
